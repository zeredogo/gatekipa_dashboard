      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto scrollbar-hide">
          {navigation.map((item) => {
            ...
            const activeColor = isFreeze ? "text-rose-400 bg-rose-500/10 border-rose-500/20" : "bg-blue-500/10 text-blue-400 border-blue-500/20";
            ...
            return (
              <Link 
                ...
                `}
              >
                <item.icon className={`w-5 h-5 flex-shrink-0 ${isActive ? (isFreeze ? 'text-rose-400' : 'text-blue-400') : `text-gray-400 ${isFreeze ? 'group-hover:text-rose-400' : 'group-hover:text-white'} transition-colors`}`} />
        ...
        <header className="h-20 glass-panel border-b border-white/5 flex items-center justify-between px-8 z-10 shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div className="relative group hidden md:block">
              <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-400 transition-colors" />
              <input 
                type="text" 
                placeholder="Search across platform..." 
                className="w-96 bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/50 focus:bg-white/10 transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 cursor-pointer hover:bg-white/5 py-1 px-2 rounded-lg transition-colors">
              ...
              <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 border-2 border-white/10 flex items-center justify-center font-bold text-white uppercase">{adminEmail.charAt(0)}</div>
            </div>
          </div>
        </header>
